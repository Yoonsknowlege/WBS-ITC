#!/usr/bin/env python3
"""
build_long_form_table.py — Construct a long-form (family x domain)
cell-level assignment table from the released family-level ledger.
...
The released `data/assignment_table.csv` is in family-level wide form
...
Output columns:
    ...
    assignment_hit         ("1" if domain in itc_codes; "0" otherwise)
    action                  ("keep"     -> rule_union_hit AND assignment_hit
                             "suppress" -> rule_union_hit AND NOT assignment_hit
                             "add"      -> NOT rule_union_hit AND assignment_hit
                             "none"     -> not rule_union_hit AND not assignment_hit)
    status_category      (carried over from the family-level ledger)
    analysis_set_443        (carried over from the family-level ledger)

Active-layer accounting derived from this long-form assignment table:
...
Usage:
    ...
    -> writes data/assignment_table_long.csv
"""
...
OUT_PATH = os.path.join(DATA_DIR, "assignment_table_long.csv")
...
def main():
    ...
    with open(TABLE_PATH, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            screening = row.get("status_category", "")
            ...
            for dom in ITC_DOMAINS:
                ...
                rows_out.append({
                    ...
                    "assignment_hit": adj_hit,
                    "action": action,
                    "status_category": screening,
    ...
    fieldnames = ["lens_id", "domain", "cpc_rule_hit", "keyword_rule_hit",
                  "rule_union_hit", "assignment_hit", "action",
                  "status_category", "analysis_set_443"]
    with open(OUT_PATH, "w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames, lineterminator="\n")
        w.writeheader()
        w.writerows(rows_out)

    print("Long-form assignment table written to:")
    print(" ", OUT_PATH)
    print()
    ...
    print("agreement within the ITC-classified subset).")
