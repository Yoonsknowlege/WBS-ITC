...
records and 1 screening-log-only record excluded); because all
...
def main():
    ...
    md.append(f"| Excluded (9 false-match + 1 screening-log-only) | 10 |")
    ...
    md.append("")
    md.append("Online Resource 1 Table S13 accounting:")
    md.append("")
    md.append("| Layer | With rescue-route classified families | Excluding 9 classified rescue-route families |")
    md.append("|---|---|---|")
    md.append("| Active analytical layer | 443 | 434 |")
    md.append(f"| ITC-classified subset    | {len(classified)} | {len(classified) - 9} |")
    md.append("")
    md.append("Note: the below-threshold screening-log record is excluded from both computations.")
    md.append("")
    ...
    md.append(f"- Change: **{j_orig:.3f} -> {j_adj:.3f}** (delta = {j_adj - j_orig:+.3f})")
    md.append("")
    md.append("Interpretation: removing E04H15-only families attenuates the 4-1 / 4-4 "
              "Jaccard somewhat, indicating that part of the habitat-deployable adjacency "
              "is driven by the shared E04H15 anchor rather than independent claim-level evidence.")
    md.append("")

    md.append("---")
    md.append("Generated by `scripts/sensitivity_checks.py` from the released "
              "`data/phase2_453_families.json` and `data/assignment_table.csv`.")
    md.append("")
    ...
    print("Sensitivity notes written to:", OUT_MD)
    print()
    print("Online Resource 1 Table S13 accounting:")
    print(f"  Active analytical layer with rescue-route classified families: 443")
    print(f"  Excluding 9 classified rescue-route families:                  434")
    print(f"  ITC-classified subset with rescue-route families:              {len(classified)}")
    print(f"  Excluding 9 classified rescue-route families:                  {len(classified) - 9}")
    print(f"  Below-threshold screening-log record excluded from both computations.")
    print()
    print("Released-level accounting (script-level):")
    print(f"  Released dataset:           {len(families)}")
    print(f"  Rescue families:            {len(rescue_fams)}")
    print(f"  Leave-out (no-rescue):      {len(no_rescue)}")
    print(f"  Maximum WBS share change:   {max_change:.2f} pp")
    print(f"  Top-six Jaccard pairs unchanged after rescue exclusion: {top6_same}")
