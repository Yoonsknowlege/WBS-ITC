...
Purpose:
    ...
    in the manuscript (main analytical accounting and rule-hit/assignment agreement outputs; body Figure 2/3 aggregation
    values, the CPC co-classification matrix, and Online Resource 1
    Table S10 source values) directly from phase2_453_families.json,
    filtered to the 443-family analytically active subset using
    assignment_table.csv. It then cross-checks them against the
    pre-computed constants stored in isru_data.py to confirm consistency.

    The numeric components of Online Resource 1 Table S12 are produced
    by scripts/table_s12_profiles.py (this script does not regenerate
    Table S12 directly).

Dataset scope:
    Released families:            453  (phase2_453_families.json)
    False-match:                    9  (unclassified; excluded)
    Screening-log-only near-threshold: 1  (unclassified; excluded)
...
from isru_data import (
    ...
    DOMAIN_EXTERNAL_ACTIVE, CATEGORY_C, SCREENING_LOG_ONLY,
...
# The assignment_table.csv provides analysis_set_443 == true/false to
...
EXPECTED_SCREENING_LOG = 1
EXPECTED_EXCLUDED = EXPECTED_FALSE_MATCH + EXPECTED_SCREENING_LOG
...
def recompute_table3(families):
    """ITC domain portfolio tag counts."""
    counter = Counter()
    total_tags = 0
    classified = 0
    for fam in families:
...
def recompute_table4_jaccard(families):
    """ITC-domain pairwise Jaccard similarity matrix (15x15)."""
...
def recompute_orS1_orS10_cpc(families):
    """Online Resource 1 Fig. S1 + Table S10 source matrix:
    CPC co-classification matrix (top 25 codes).
    """
...
def main():
    ...
    print(f"Excluded (from table):         {n_excluded}  ({EXPECTED_FALSE_MATCH} false-match + {EXPECTED_SCREENING_LOG} screening-log-only)")
    ...
    print()
    print("NOTE: ITC metrics (main analytical accounting and rule-hit/assignment")
    print("          diagnostic outputs) computed on full 453")
    print("          (invariant —")
    print("      all excluded families lack ITC tags).")
    print("      CPC metrics (Online Resource 1 Fig. S1 / Table S10 source matrix and bridging) are computed on the 443-active subset")
    ...
    # ── Jaccard co-tagging outputs (Jaccard top-pair table and Fig. 4 source) ──
    # (Computed on full 453; ITC metrics mathematically invariant)
    ...
    # ── Online Resource 1 Fig. S1 + Table S10 source: CPC co-classification ──
    # (Computed on the 443-active subset)
    cpc_cooc = recompute_orS1_orS10_cpc(families_active)
    cpc_match = np.array_equal(cpc_cooc, CPC_COOCCURRENCE)
    print(f"\n--- OR Fig. S1 / Table S10 source: CPC Co-classification (N=443 active) ---")
