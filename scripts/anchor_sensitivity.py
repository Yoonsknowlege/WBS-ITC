...
For each of the top-10 Jaccard co-tagging pairs in the assignment layer:
...
def get_domain_sets(families):
    """Build domain -> set of family indices mapping from the assignment ITC codes."""
...
def interpret(delta, j_adj):
    # Wording aligned with Online Resource 1 Table S8 footnote framing:
    # "Least attenuated", "Attenuated; residual overlap", "Strongly
    # conditioned", "Moderately conditioned". Read-out is descriptive
    # of classification-space adjacency only, not engineering coupling.
    if abs(delta) < 0.03:
        return "Least attenuated"
    elif j_adj > 0.10:
        return "Attenuated; residual overlap"
    elif abs(delta) > 0.10:
...
def main():
    ...
    print("  J = raw Jaccard similarity on assignment ITC tags")
    ...
    print("  'Strongly conditioned': J_adj <= 0.10 and abs(delta) > 0.10 (signal driven by anchor)")
    print("  'Moderately conditioned': intermediate cases")
