...
This script:
  ...
  1. Filters data/assignment_table.csv to the 70 active domain-external
     families (status_category = domain_external_active).
...
incorporates an author-level review of each family's first-claim
mechanism, which is author-reviewed rather than rule-derivable. The categories
...
Mapping rule (priority order; each family counted once):
    ...
    Boundary-adjacent (rule)  -> any of {H02J, H02S, Y02E, Y02P, Y02A,
                                         B32B, F04B, F03G}
    Other / author-reviewed   -> none of the above prefix sets matched

The total equals 70 (the active domain-external count) by construction.
The "Other / author-reviewed" bucket corresponds to families that
...
def categorise(prefixes):
    for label, prefix_set in PARTITION_RULES.items():
        if prefixes & prefix_set:
            return label
    return "Other / author-reviewed"


def main():
    # 1) Identify the 70 active domain-external lens_ids from the assignment table
    domain_external_ids = set()
    with open(TABLE_PATH, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            if row.get("status_category") == "domain_external_active":
    ...
    print("uses an author-reviewed first-claim partition that is not script-")
    print("generated.")
    print()
    print("Active domain-external families (assignment_table.csv):", len(domain_external_ids))
    ...
    counts["Other / author-reviewed"] = 0
    ...
    print("Table S7a Panel A reports the authors' first-claim review, which")
    print("redistributes the 'Other / author-reviewed' bucket above between")
    print("Table S7a's 'Boundary-adjacent' (22) and 'Unclassified' (5)")
    print("categories. The 70-family total is reproducible; the")
    print("author-reviewed subdivision into Boundary-adjacent vs Unclassified is")
