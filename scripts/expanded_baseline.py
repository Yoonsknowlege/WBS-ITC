...
layer:
  ...
  - Keyword-rule only       (NEW; compares against rule-hit agreement
  ...
  - Released assignment  (already in Table S14)

For each variant, reports:
  - N classified families
  - Total tags
  - Assignment-tag overlap vs released assignment layer (shared / 795)
  - Variant-positive overlap (shared / variant total tags)
  - WBS-layer tag shares
  - Top-6 Jaccard overlap with the released assignment top-6
...
def compute_layer(assigns, label, ref_tag_set=None):
    ...
    If ref_tag_set is supplied, also compute assignment-tag overlap (shared/795)."""
    tag_count = Counter()
    n_class = 0
    ...
    # Assignment-tag overlap vs the released assignment layer (shared / 795)
...
def main():
    ...
    # Build per-family rule-only / keyword-only / rule-union / assignment tag sets
    cpc_only = []
    kw_only = []
    rule_union = []
    assignment = []
    ...
    # Build ref_tag_set from the released assignment for overlap computation
    ref_tag_set = set()
    for i, doms in enumerate(assignment):
        for d in doms:
            ref_tag_set.add((i, d))
    ...
    layers = [
        ...
        compute_layer(explicit_lunar_layer,
                                          ...
                                          ref_tag_set),
        compute_layer(assignment,        'Released assignment',  ref_tag_set),
    ]
    ...
    print('on the 443-family active layer. Released assignment layer is the reference.')
    print('=' * 105)
    print()
    header = ('%-44s %5s %6s %9s %10s %6s %6s %6s %6s %9s' % (
        'Layer', 'N cl.', 'Tags', 'Asg ovl.', 'Var ovl.',
        'WBS-1', 'WBS-2', 'WBS-3', 'WBS-4', 'Top-6 ov.'))
    print(header)
    print('-' * 105)
    for L in layers:
        if L['label'].startswith('Released assignment'):
            tag_rec_str = 'ref'
            retention_str = 'ref'
    ...
    print('-' * 105)
    print()
    print('Note. "Asg ovl." = shared / 795 against the released assignment layer.')
    print('"Var ovl." = shared / variant total tags (variant-positive overlap).')
    print('"Top-6 ov." = number of the released-assignment top-6 Jaccard pairs that')
