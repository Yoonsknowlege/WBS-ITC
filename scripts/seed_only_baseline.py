...
443-family analytically active layer:
  ...
  - Released assignment (claim-reviewed ITC layer)
...
This is a rule-vs-assignment portfolio diagnostic, NOT a classifier-performance
evaluation. It illustrates how the framework-first WBS-ITC assignment layer
...
def main():
    table = os.path.join(DATA_DIR, 'assignment_table.csv')
    ...
    rule_union = [parse_doms(r['cpc_rule_domains']) | parse_doms(r['keyword_rule_domains'])
                  for r in active]
    assignment = [parse_doms(r['itc_codes']) for r in active]

    layers = [
        compute_layer(cpc_only, 'CPC-rule only'),
        compute_layer(rule_union, 'CPC-or-keyword (rule union)'),
        compute_layer(assignment, 'Released assignment'),
    ]

    print('=' * 90)
    print('Online Resource 1, Table S14: Within-layer comparison of deterministic')
    print('first-pass and claim-reviewed assignment outputs on the 443-family active layer')
    ...
    print('--- Reading ---')
    print('Claim review produces a materially different WBS profile from rule-only')
    ...
    print('larger in the assignment layer (J = 0.124 -> 0.341), and the')
    print('habitat-envelope Jaccard value (4-1 <-> 4-4) is 2.31x larger in the')
    print('assignment layer (J = 0.127 -> 0.294). The CPC-rule-only top pair')
    print('3-3 <-> 4-1 (J = 0.162) does not appear in the assignment top-6,')
    ...
    print('evaluation. The assignment layer is the operational reference layer for')
    print('this study, as bounded in the claim-boundary and discussion sections of the main text;')
    print('this comparison illustrates what claim review adds and removes relative to')
